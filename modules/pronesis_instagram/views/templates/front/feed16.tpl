...
*}
<h4 class="text-center instagram-feed-title">{l s='Instagram posts' mod='pronesis_instagram'}</h4>
<div class="instagram-feed">
  <div id="instagram-feed-carousel" class="instagram-feed-carousel">
    ...
    <div class="instagram-feed-carousel-item">
          ...
          <figcaption>
            <h5 class="hidden-xs">{if isset($image.caption)}{$image.caption|truncate:30:"...":true}{/if}<small> ({$image.timestamp|date_format:"%A %e %B %Y"}) - <a href="{$image.permalink}" target="_blank" rel="nofollow noopener">{l s='Go to Instagram' mod='pronesis_instagram'}</a></small></h5>
          </figcaption>
        </div>
    {/foreach}          
  </div>
