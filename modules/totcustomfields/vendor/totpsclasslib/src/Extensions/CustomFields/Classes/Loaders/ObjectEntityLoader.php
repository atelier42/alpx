...
class ObjectEntityLoader extends EntityLoader
{
    protected static $entities = [];

    protected static $isLoaded = false;

    protected $addHookName = 'addTotcustomfieldsCustomFieldsObject';

    protected $skipHookName = 'skipTotcustomfieldsCustomFieldsObject';

    public function addEntity($class)
    {
        /** @var CustomFieldsObject $instance */
        $instance = new $class();
        $name = $instance->getObjectCode();
        self::$entities[$name] = [
            'class' => $class,
            'instance' => $instance,
        ];
    ...
    public function removeEntity($class)
    {
        /** @var CustomFieldsObject $instance */
        $instance = new $class();
        if (array_key_exists($instance->getObjectCode(), self::$entities)) {
            unset(self::$entities[$instance->getObjectCode()]);
        }
    }

    protected function loadDefaultEntities()
    {
        $this->addEntity(CustomFieldsObjectCategory::class);
        $this->addEntity(CustomFieldsObjectOrder::class);
        $this->addEntity(CustomFieldsObjectProduct::class);
    }
}
