...
class CustomFieldsInputVideoService
{
    /**
     * Returns an existing TotCustomFieldsInputVideoValue from the parameters, or a new object filled with their values if none was found
     *
     * @param int $idInput
     * @param int $idObject
     * @param int|null $idLang if null, return non-translatable value
     *
     * @return CustomFieldsInputVideoValue
     *
     * @throws \PrestaShopDatabaseException
     * @throws \PrestaShopException
     */
    public static function getInputValue($idInput, $idObject, $idLang)
    {
        $query = new DbQuery();
        $query->select('id_input_value');
        $query->from('totcustomfields_input_video_value');
        ...
        if ($res) {
            return new CustomFieldsInputVideoValue((int) $res);
        }

        $newInputValue = new CustomFieldsInputVideoValue();
    ...
    /**
     * Returns all translatable TotCustomFieldsInputVideoValue for an object
     *
     * @param int $idInput
     * @param int $idObject
     *
    ...
    public static function getInputTranslatableValues($idInput, $idObject)
    {
        ...
        $inputValues = [];
        foreach ($res as $row) {
            $inputValues[] = new CustomFieldsInputVideoValue((int) $row['id_input_value']);
        }
     ...
     * @param int $idInput
     *
     * @return array an array of TotCustomFieldsInputVideoValue
     *
     * @throws \PrestaShopDatabaseException
     * @throws \PrestaShopException
     */
     ...
     * @return bool
     */
    public static function copyUntranslatedValuesToDefaultLanguage($idInput)
    {
        $idLangDefault = (int) Configuration::get('PS_LANG_DEFAULT');

        $sqlQuery = '
                    INSERT INTO `' . _DB_PREFIX_ . 'totcustomfields_input_video_value' . '` (id_input, id_object, id_lang, value)
                    SELECT id_input, id_object, ' . (int) $idLangDefault . ', value
                    FROM `' . _DB_PREFIX_ . 'totcustomfields_input_video_value' . '`
                    WHERE id_input = ' . (int) $idInput . '
                    AND id_lang IS NULL
                    AND NOT EXISTS (
                        SELECT id_input FROM `' . _DB_PREFIX_ . 'totcustomfields_input_video_value' . '`
                        WHERE id_input = ' . (int) $idInput . '
