...
include_once('sx_slider_ingredient.php');

$home_slider = new Sx_slider_ingredient();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'ingredientslider_slides` SET `position` = '.(int)$position.'
			WHERE `id_ingredientslider_slides` = '.(int)$id_slide
		);
