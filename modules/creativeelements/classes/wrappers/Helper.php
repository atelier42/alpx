...
function add_action($tag, $callback, $priority = 10)
{
    ...
    if (is_array($callback) && isset($callback[1]) && strrpos($callback[1], '_')) {
...
if (is_admin() || \Tools::getValue('render') === 'widget') {
    ...
    function wp_enqueue_style($handle, $src = '', array $deps = [], $ver = false, $media = 'all')
    {
        ...
        if (!empty(Helper::$styles[$handle])) {
            ...
            $path = $args['src'];
            $params = [
                'server' => strrpos($path, '://') === false ? 'local' : 'remote',
                'media' => $args['media'],
            ];
...
function wp_print_styles()
{
    while ($args = array_shift(Helper::$head_styles)) {
        ...
        if ($args['ver']) {
            $args['src'] .= (strrpos($args['src'], '?') === false ? '?' : '&') . 'v=' . $args['ver'];
        }
...
function parse_shortcode($match)
{
    ...
    if ('$' === $match[2][0]) {
        ...
        } elseif (is_array($result) || $result instanceof \PrestaShop\PrestaShop\Adapter\Presenter\AbstractLazyArray) {
            $array = [];
...
function parse_var($var)
{
    ...
    for ($i = 1; $i < $count; $i++) {
        ...
        if ('.' === $prefix && is_array($result) && isset($result[$key])) {
            $result = $result[$key];
