...
include_once dirname(__FILE__) . '/product-variants.php';
...
class WidgetProductMiniatureVariants extends WidgetProductVariants
{
    ...
    protected function render()
    {
        ...
        <?php foreach ($product['main_variants'] as $i => $variant) : ?>
            <?php if ($i < $limit) : ?>
                <a class="ce-product-variants__pattern ce-product-variants__<?= esc_attr($variant['type']) ?>"
                    href="<?= esc_attr($variant['url']) ?>" title="<?= esc_attr($variant['name']) ?>"
                <?php if ($variant['html_color_code']) : ?>
                    style="background-color: <?= esc_attr($variant['html_color_code']) ?>"
                <?php elseif ($variant['texture']) : ?>
                    style="background-image: url(<?= esc_attr($variant['texture']) ?>)"
                <?php endif ?>></a>
            <?php else : ?>
                <span class="ce-product-variants__pattern ce-product-variants__count">
                    +<?= count($product['main_variants']) - $limit ?>
                </span>
                <?php break ?>
            <?php endif ?>
        <?php endforeach ?>
    ...
    protected function renderSmarty()
    {
        ...
        {foreach $product.main_variants as $i => $variant}
            ...
            {else}
                <span class="ce-product-variants__pattern ce-product-variants__count">
                    +{$product.main_variants|count - <?= $limit ?>}
                </span>
                {break}
            {/if}
        {/foreach}
