...
class ModulesXCatalogXModule extends BaseModule
{
    ...
    private function shouldRegister()
    {
        ...
        if (null === $should_register) {
            ...
            $should_register = \Tools::getIsset('submitOptionsce_template') ||
    ...
    public function refreshProduct($content)
    {
        ...
        $getIPA = version_compare(_PS_VERSION_, '1.7.3.1', '<')
            ? 'getIdProductAttributesByIdAttributes'
            : 'getIdProductAttributeByIdAttributes';
        $ipa = $groups ? (int) call_user_func("Product::$getIPA", $id_product, $groups, true) : null;
    ...
    private static function filterElementsData(array &$data)
    {
        foreach ($data as &$value) {
            ...
            } elseif (is_string($value) && strrpos($value, '}') !== false) {
    ...
    {
        if ('publish' === $data['settings']['post_status'] &&
            'product-miniature' === $document->getTemplateType() &&
            UID::THEME === get_the_ID()->id_type
        ) {
            ...
            add_action('elementor/element/after_add_attributes', function ($element) {
                ...
                if ($settings && strrpos($settings[0], '}') !== false) {
