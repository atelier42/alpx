...
module.exports = Marionette.Behavior.extend({
	...
	renderTools: function renderTools() {
		var dynamicSettings = this.getOption('dynamicSettings');

		if (dynamicSettings.default && !dynamicSettings.defaultEditable) {
			return;
		}
	...
	onTagsListItemClick: function onTagsListItemClick(event) {
		...
		if (this.tagView.hasSettings() && this.tagView.getTagConfig().settings_required) {
...
module.exports = Marionette.ItemView.extend({
	...
	getTemplate: function getTemplate() {
		var config = this.getTagConfig(),
			...
			isRemovable: !dynamicSettings.default || dynamicSettings.defaultEditable,
			title: config.title,
...
ControlMediaItemView = ControlMultipleBaseItemView.extend({
	ui: function ui() {
		...
		ui.urlField = 'input[id^=elementor-control-media-url]';
...
module.exports = elementorModules.Module.extend({
	...
	loadCacheRequests: function loadCacheRequests() {
		...
		Backbone.$.post(elementor.config.document.urls.preview, {
			render: 'tags',
			editor_post_id: elementor.config.document.id,
			data: JSON.stringify({
				post_id: elementor.config.document.id,
	...
	tagTextToTagData: function tagTextToTagData(tagText) {
		var tagMatch = tagText.match(/id="(\w*)" name="([\w\-]*)" settings="(.*?(?="]))/);

		return !tagMatch ? false : {
			id: tagMatch[1],
			name: tagMatch[2],
			settings: JSON.parse(decodeURIComponent(tagMatch[3]))
		};
	},
...
module.exports = BaseSettings.extend({
	changeCallbacks: {
		elementor_page_title_selector: function elementor_page_title_selector(newValue) {
			var newSelector = newValue || 'header.page-header',
		...
		elementor_page_wrapper_selector: function elementor_page_wrapper_selector(newValue) {
			var newSelector = newValue || '#content, #wrapper, #wrapper .container',
